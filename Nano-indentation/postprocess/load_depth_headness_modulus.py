...
def main():
    ...
    if use_smoothed_for_Pmax and smooth_window > 1:
        P_series_for_max = simple_smooth(
            pd.concat([dfL["Load"], dfU["Load"]], ignore_index=True).values, smooth_window
        )
        D_series_for_max = simple_smooth(
            pd.concat([dfL["Depth"], dfU["Depth"]], ignore_index=True).values, smooth_window
        )
        idx_pmax = int(np.argmax(P_series_for_max))
        Pmax = float(P_series_for_max[idx_pmax])
        hmax = float(D_series_for_max[idx_pmax])
    else:
        df_all = pd.concat([dfL[["Depth","Load"]], dfU[["Depth","Load"]]], ignore_index=True)
    ...
    df_plot = pd.concat([dL, dU], ignore_index=True)
